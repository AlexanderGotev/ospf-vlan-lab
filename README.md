...
If this folder isn't a git repo yet, run these commands from inside it:
...
git init
git add .
git commit -m "Initial commit: OSPF + VLAN routed lab (3 routers, 3 sites)"

# Create the repo on GitHub first (via github.com → New Repository),
# then link your local repo to it — replace the URL with your own repo's URL
git remote add origin https://github.com/<your-username>/ospf-vlan-lab.git
...
**For future updates** (e.g. after adding new configs or notes):
```bash
git add .
...
**Tip:** if you don't want to create the GitHub repo manually first, you can instead use the GitHub CLI (`gh`) to do it in one step:
```bash
gh repo create ospf-vlan-lab --public --source=. --remote=origin --push
```
